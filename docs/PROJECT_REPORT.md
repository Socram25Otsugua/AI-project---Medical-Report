...
<p class="cover-kicker">Computer Science — AI apps</p>

<h1 class="cover-title">Offline AI Radio Medical Report Reviewer</h1>

<p class="cover-meta"><strong>Author:</strong> Marcos Augusto</p>
<p class="cover-meta"><strong>Date:</strong> May 2026</p>
<p class="cover-meta"><strong>GitHub:</strong> <a href="https://github.com/Socram25Otsugua/AI-project---Medical-Report">github.com/Socram25Otsugua/AI-project---Medical-Report</a></p>

<img src="assets/frontend-screenshot.png" alt="Radio Medical Assistant — structured ABCDE workflow with AI doctor chat" class="cover-screenshot" />

<p class="cover-caption"><em>Figure 0.1 — Radio Medical Assistant: Danish Maritime sidebar, eight-step ABCDE form, observation chart in the center panel, and dedicated AI doctor chat.</em></p>
...
**My goal:** Build a reproducible, offline pipeline that ingests a trainee report, enriches it with protocol context (RAG + MCP), produces structured JSON feedback through LangChain chains, and exposes it via a React UI with session history — while keeping a human instructor in the loop for final judgment.

### 1.3 Frontend workflow (current design)

The React UI follows a single-session workflow inspired by Danish Maritime Radio Medical operations:

| Area | Role |
|------|------|
| **Sidebar** | New report, search, saved history with clinical/completeness badges, on-duty profile |
| **Stepper (8 steps)** | P — Patient, A–E ABCDE, Problem, Actions |
| **Center panel** | Structured form; after first send to the AI doctor the form locks and the **observation chart** stays here |
| **Right panel** | **AI doctor chat only** — no observation chart embedded in the chat box |
| **Header** | Live session badge, progress tracker (step *n* / 8), **View clinical summary** button |
| **Summary view** | Clinical dashboard: patient hero banner, vitals grid, ABCDE recap, working impression |

Trainees complete the ABCDE form and mandatory first-column observation vitals, send the case to the AI doctor, continue the dialogue in chat, add repeat observations in the center chart, and open the **clinical summary** when follow-up questions are resolved.

### 1.4 Architecture summary

| Layer | Technology | Role |
|-------|------------|------|
| Frontend | React (Vite + TypeScript) | Stepper form, observation chart, chat, clinical summary, history |
| Backend | FastAPI | REST API, orchestration entry points |
| LLM | Ollama (`llama3.1` default) | Form review, summary actions, patient eval, chat |
...
- `app/services/form_analysis/` — form review chain (`analyze_form`)
- `app/services/chat/` — AI doctor turns and conversation state
- `app/services/summary/` — summary actions, patient evaluation, and `build_clinical_summary`
...
I selected **Ollama** as the inference runtime because it satisfies the offline requirement, supports structured JSON generation, and integrates cleanly with LangChain (`ChatOllama`, `OllamaLLM`, `OllamaEmbeddings`).
...
|-------|---------------|-------------|-----------|
| Form review / Summary actions / Patient eval | `OLLAMA_MODEL` | 0.2 | Structured JSON; balance consistency and nuance |
...
**Why not fine-tuning immediately?** The domain knowledge is largely procedural (ABCDE, Danish maritime telemedicine). I inject that knowledge via **RAG** and **MCP** rather than weight updates, reducing data and compute requirements.
...
### 2.3 Custom model: my approach and reasoning

I provide an **optional custom Ollama model** (`radio-medical-ai`) via a Modelfile that layers a domain system prompt and temperature on top of `llama3.1`. In production configuration I **default to the stock model** and rely on application-level system prompts, RAG, and MCP enrichment.
...
**Arguments against (why I kept it optional)**
...
**Conclusion:** I document and ship the custom Modelfile for teams that want a packaged “Radio Medical AI” persona, but use stock `llama3.1` during development and Promptfoo CI unless profiling shows a measurable gain.
...
| Prompt | File | Purpose |
|--------|------|---------|
| Form review | `backend/app/prompts/form_review_prompt.py` | ABCDE deficiency review, completeness score, safety flags |
| Summary actions | `backend/app/prompts/summary_actions_prompt.py` | Immediate actions, monitoring, escalation, follow-up questions |
| Patient evaluation | `backend/app/prompts/summary_patient_eval_prompt.py` | Patient status (`ok` / `concerning` / `critical` / `unknown`) |
| Chat doctor | `backend/app/services/chat/chat_turn_service.py` | Radio Medical Denmark role-play with case status |
...
- Chat doctor must not ask generic standing instructions already covered by an active conversation.

### 3.2 Promptfoo setup

Prompt contract tests live in `promptfoo/` (see `promptfoo/README.md`) and run in CI (`.github/workflows/pr-checks.yml`). I use the **`ollama:chat:llama3.1`** provider with `temperature: 0.2` and `format: json`.
...
Although optional, I ship a **Modelfile** at `ollama/Modelfile`:
...
The custom model does **not** replace application prompts — it sets a baseline persona. Form review, summary actions, and patient-eval chains still attach their full system prompts and JSON schemas. This layering lets operators swap the base tag while preserving chain-specific instructions.
...
| `POST` | `/api/v1/reports/analyze` | `AnalyzeResult` | Full pipeline: review + summary actions + patient evaluation |
...
I use MCP in two complementary ways: **deterministic FastMCP tools** for structured extraction, and **MCP-style context helpers** for RAG and session memory.
...
This bundle is injected into review, summary, and patient-eval chains so the LLM receives **evidence-backed** context rather than guessing completeness.
...
After LLM output, I apply **rule-based filters**:
...
- **Question deduplication** — Skip consciousness questions when AVPU/GCS already documented; filter generic “report any changes” chat questions.
...
I considered a dedicated **judge chain** that re-scores or rejects primary outputs. The current design **embeds judgment into the review and patient-eval prompts** instead of a separate judge pass, to limit latency on local hardware.
...
**Trade-off:** A separate judge model (e.g. “critic” pass) would improve safety at ~2× inference cost. For offline training, I prioritised deterministic MCP + post-filters.
...
2. **Chat** — AI doctor asks follow-ups; `can_finalize_summary` is false while `pending_questions` remain. The observation chart is edited in the center panel, not inside the chat box.
3. **Summary** — Full analyze runs when the trainee/instructor opens **View clinical summary**; results can be saved to history with clinical and completeness badges.
...
I delivered an offline Radio Medical training assistant that combines **LangChain orchestration**, **local RAG**, **MCP structured tools**, and a **React ABCDE workflow** with dedicated chat, observation chart, and clinical summary views. The architecture addresses the core problem: scalable, consistent feedback on trainee documentation and clinical reasoning without cloud dependency.

**Key outcomes:**

- Structured JSON API for form review, summary actions, patient status, and conversational turns
- Service modules split into form analysis, chat, and summary generation for clearer maintenance
...
- Clinical summary dashboard with vitals quality dial, ABCDE recap, and working impression
...
### A.1 Form review system prompt (excerpt)

Source: `backend/app/prompts/form_review_prompt.py`
...
### A.2 Summary actions prompt (excerpt)

Source: `backend/app/prompts/summary_actions_prompt.py`
...
Source: `backend/app/prompts/summary_patient_eval_prompt.py`
...
Source: `backend/app/services/chat/chat_turn_service.py`

- Persona: Radio Medical Denmark
- Output JSON: `assistant_message`, `pending_questions`, `can_finalize_summary`
...
This report is **Markdown** and editable in Cursor, VS Code, or any text editor.

### Option 1 — Project export script (recommended)
...
This writes `docs/PROJECT_REPORT.pdf` with the cover screenshot embedded.

To refresh the screenshot first (frontend must be running on port 5173):
...
1. View the rendered Markdown on GitHub or paste into [StackEdit](https://stackedit.io).
2. **Print → Save as PDF** with background graphics enabled.

### Before submission checklist

- [x] Course and student name filled in on the front page
- [x] Screenshot reflects the current Radio Medical Assistant UI
